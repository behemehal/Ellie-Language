...
var log = (message, color) => { console.log(`Ellie: ${message}`); }
...
var version = cargoconf.split(whichLineEnding(cargoconf))
                  .find(x => x.split("=")[0].trim() == "version")
                  .split("=")[1]
                  .trim()
                  .replace(new RegExp("\"", "g"), "");
var dbgversion = ellidbg.split(whichLineEnding(ellidbg))[0].split(":")[1];
var changeHeaders = dbgversion.split(".")[0] != version.split(".")[0] ||
                    dbgversion.split(".")[1] != version.split(".")[1]
...
function refactorFile(file, fileDir) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i];
    if (line.includes("debug_message: \"") &&
        fileDir != "./core/src/error/mod.rs") {
      var dbgId = createDebugLabel();
      debugLabels += "|  " + dbgId + "  :  " + fileDir + ":" + (i + 1) + eol;
      var first = line.split("debug_message: \"")[0];
      factoredFile += first + "debug_message: \"" + dbgId + "\"" +
                      line.split("debug_message: \"")[1].split("\"")[1] +
                      ((lines.length - 1) == i ? "" : eol)
      factored = true;
    } else {
  ...
  return new Promise(async (resolve) => {
    for (var i = 0; i < dir.length; i++) {
      if (dir[i].isFile()) {
        ...
        q = await scanDirectory(
            fs.readdirSync(path + dir[i].name, {withFileTypes : true}),
            path + dir[i].name + "/");
        files = [...q, ...files ];
      }
    }
...
if (!changeHeaders) {
  ...
  scanDirectory(fs.readdirSync("./", {withFileTypes : true}), "./")
      .then((files) => {
