...
fn main() {
    ...
    if allow_build {
        ...
        .unwrap();
        let mut map_content = format!("//Auto builded from `instructions.csv` by `build.rs`\nuse std::collections::HashMap;\nuse lazy_static;\npub static Revision : i8 = {revision};\n\n#[derive(Clone)]\npub struct Instruction {{\n\tpub rtype: &'static str,\n\tpub code: u8,\n\tpub mode: &'static str,\n}}\n\nlazy_static! {{\n\tpub static ref INSTRUCTIONS : HashMap<&'static str, Instruction> = {{\n\t\tlet mut i = HashMap::new();\n");

        for entry in instruction_entries {
            map_content.push_str(&format!(
            "\t\ti.insert(\"{}_{}\",Instruction {{\n\t\t\trtype: \"{}\",\n\t\t\tcode: {},\n\t\t\tmode: \"{}\",\n\t\t}});\n",
            entry.0, entry.1, entry.0, entry.2, entry.1
        ));
        }
        map_content.push_str("\t\ti\n\t};\n}\n");
