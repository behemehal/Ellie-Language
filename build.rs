...
fn main() {
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            ...
            ellie_engine_version = ellie_lang_toml["package"]["version"].clone();
            ellie_engine_version_name = ellie_lang_toml["package"]["version_code"].clone();
        }
        Err(_) => {
            panic!(
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/core/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            let ellie_lang_toml = cargo_toml.parse::<Value>().unwrap();
            ellie_core_version = ellie_lang_toml["package"]["version"].clone();
        }
        Err(_) => {
            panic!(
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/tokenizer/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            let ellie_lang_toml = cargo_toml.parse::<Value>().unwrap();
            ellie_tokenizer_version = ellie_lang_toml["package"]["version"].clone();
        }
        Err(_) => {
            panic!(
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/parser/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            let ellie_lang_toml = cargo_toml.parse::<Value>().unwrap();
            ellie_parser_version = ellie_lang_toml["package"]["version"].clone();
        }
        Err(_) => {
            panic!(
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/bytecode/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            let ellie_lang_toml = cargo_toml.parse::<Value>().unwrap();
            ellie_bytecode_version = ellie_lang_toml["package"]["version"].clone();
        }
        Err(_) => {
            panic!(
    ...
    match read_file(env!("CARGO_MANIFEST_DIR").to_owned() + &"/vm/Cargo.toml".to_owned()) {
        Ok(cargo_toml) => {
            let ellie_lang_toml = cargo_toml.parse::<Value>().unwrap();
            ellie_vm_version = ellie_lang_toml["package"]["version"].clone();
        }
        Err(_) => {
            panic!(
