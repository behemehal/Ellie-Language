...
impl super::InstructionExecuter for LDB {
    fn execute(
        &self,
        ...
        match &addressing_value {
            AddressingValues::Immediate(raw_type) => {
                current_stack.registers.B = if raw_type.type_id.is_stack_storable() {
                    *raw_type
                } else {
            ...
            AddressingValues::Absolute(e) => {
                current_stack.registers.B = match stack_memory
                    ...
                    Some(stack_data) => {
                        if stack_data.type_id.is_heap_reference() {
                            ...
                            if index > array_size - 1 {
                                ...
                                current_stack.registers.B = match stack_memory.get(&(entry + 2)) {
                                    Some(e) => e,
                                    None => {
                                        return Err(ExecuterPanic {
                        ...
                        match heap_memory.get(&static_raw_type.to_uint()) {
                            Some(raw_type) => {
                                if raw_type.type_id.is_array() {
                                    ...
                                    current_stack.registers.B = StaticRawType::from_heap_reference(
            ...
            AddressingValues::AbsoluteStatic(e) => {
                ...
                current_stack.registers.B = match instruction.addressing_value {
            ...
            AddressingValues::IndirectA => {
                current_stack.registers.B = current_stack.registers.A;
            }
            AddressingValues::IndirectC => {
                current_stack.registers.B = current_stack.registers.C;
            }
            AddressingValues::IndirectX => {
                current_stack.registers.B = current_stack.registers.X;
            }
            AddressingValues::IndirectY => {
                current_stack.registers.B = current_stack.registers.Y;
            }
            _ => {
                return Err(ExecuterPanic {
                    reason: ThreadPanicReason::IllegalAddressingValue,
