...
impl super::InstructionExecuter for STY {
    fn execute(
        &self,
        ...
        match &addressing_value {
            AddressingValues::Implicit => {
                stack_memory.set(&current_stack.get_pos(), current_stack.registers.Y);
            }
            AddressingValues::Immediate(raw_type) => {
                stack_memory.set(&current_stack.get_pos(), *raw_type);
            }
            AddressingValues::Absolute(e) => {
                stack_memory.set(&(e + current_stack.frame_pos), current_stack.registers.Y);
            }
                    ...
                    Some(stack_data) => {
                        if stack_data.type_id.is_heap_reference() {
                            match heap_memory.get_mut(&(stack_data.to_int() as usize)) {
                                Some(heap_data) => {
                                    ...
                                    if type_id.is_array() {
                                        ...
                                        let register_bytes = current_stack.registers.Y.to_bytes();
                            ...
                            if index >= array_size {
                                ...
                                stack_memory.set(&(entry + 2), current_stack.registers.Y);
                                return Ok(ExecuterResult::Continue);
                            }
                        } else {
                            return Err(ExecuterPanic {
