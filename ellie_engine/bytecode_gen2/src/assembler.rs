...
impl InstructionPage {
    pub fn assign_instruction(&mut self, instruction: Instruction) {
        self.instructions.push(instruction)
    }

    pub fn extend_instructions(&mut self, instruction: Vec<Instruction>) {
...
impl AssembleResult {
    pub fn render_binary_to_vector(&self) -> Vec<u8> {
        ...
        for instruction in &self.instructions {
            binary.write(&instruction.to_bytes()).unwrap();
        }
        binary
    }
