            ...
            if letter_char == ";" {
                if parser.options.ignore_imports {
                    ...
                    if !response.found {
                        ...
                        match response.file_content.clone() {
                            parser::ResolvedFileContent::PreBuilt(_) => todo!(),
                            parser::ResolvedFileContent::Raw(content) => {
                                std::println!("BUILDED {:#?}", parser.built_libraries);
                                match parser
                                    .built_libraries
                                    .clone()
                                    .into_iter()
                                    .find(|x| x.0 == response.resolved_path)
                                {
                                    Some((_, inner_parser)) => {
                                        if inner_parser.syntax_errors.len() != 0 {
                                            ...
                                            errors.push(error::Error {
                                                ...
                                                message: error::errorList::error_s33
                                                    .message
                                                    .clone(),
                                                builded_message: error::Error::build(
                                    ...
                                    None => {
                                        let inner_parser = parser_clone
                                            .clone()
                                            .read_module(content, response.resolved_path.clone());

                                        parser
                                            .built_libraries
                                            .push((response.resolved_path, inner_parser.clone()));

                                        std::println!("PUSH {:#?}", parser.built_libraries);

                                        let mut newErrors = Vec::new();
                                        for i in inner_parser.syntax_errors.iter() {
                                            if i.code != 0x05 {
                                                newErrors.push(i.clone());
                                            }
                                        }
