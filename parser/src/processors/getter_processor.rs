...
pub fn collect_getter_value<F, E>(
        ...
        + Sized,
{
    ...
    if let parser::Collecting::Getter(ref mut getter_data) = parser.current {
                ...
                && ((last_char != " " && last_char != "\n") || getter_data.data.name.is_empty())
            {
                ...
                if utils::is_reserved(&getter_data.data.name) {
                    errors.push(error::Error {
                        ...
                        debug_message: "a86bf70f9ad37ff9e6ae82a344f35d83".to_owned(),
                ...
                errors.push(error::Error {
                    path: parser.options.path.clone(),
                    scope: parser.scope.scope_name.clone(),
                    debug_message: "75c3c5faa45b3437d00ce58e28702f04".to_owned(),
            ...
            && letter_char == "}"
        {
            ...
            for item in getter_data.inside_code.collected.clone() {
                match item {
                    ...
                    parser::Collecting::Ret(return_item) => {
                        if !ret_found {
                            ret_found = true;
                            if return_item.value.clone().to_definer() != getter_data.data.rtype {
                                errors.push(error::Error {
                                    path: parser.options.path.clone(),
                                    scope: parser.scope.scope_name.clone(),
            ...
            if !ret_found && getter_data.data.rtype.raw_name() != "void" {
                errors.push(error::Error {
                    path: parser.options.path.clone(),
                    scope: parser.scope.scope_name.clone(),
                    ...
                    builded_message: error::Error::build(
                        ...
                        vec![
                            ...
                            error::ErrorBuildField {
                                key: "token1".to_owned(),
                                value: getter_data.data.rtype.raw_name_with_extensions(),
                            },
                        ],
                    ),
                    pos: getter_data.data.rtype_pos,
                });
            }

            getter_data.data.code = filtered_items;
